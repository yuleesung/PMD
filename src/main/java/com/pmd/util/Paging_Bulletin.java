...
private int nowPage, rowTotal/*총게시물 수*/, blockList/*한 페이지에 표현될 게시물 수*/, blockPage/*한 블럭당 표현할 페이지 수*/, totalPage, startPage, endPage, begin, end;
	...
	public Paging_Bulletin(int nowPage, int rowTotal, int blockList, int blockPage, String b_category) {
		super();
	...
	public void makeHTML() {
		...
		sb = new StringBuffer("<ul class='pagination pagination-v4'>");
		...
		for(int i=startPage; i<=endPage; i++) {
			// i의 값이 현재 페이지 값과 같을 때는 a태그를 지정하지 않고 숫자만 출력한다.
			if(i == nowPage) {
